...
Targets host/shared/protocol.py's real wire-format encoder directly - the
Phase-1 plan's stub sink is no longer needed since the real transport now
exists. protocol.py is a flat sibling in this same shared/ directory (see
that directory's own note on why everything here is flat, not packaged) -
no sys.path bootstrapping needed to reach it.
"""
...
class HidCommandSink:
    ...
    def send(self, command: Command) -> None:
        ...
        # from the caller's side (no exception - runner.py's own
