...
injection seams engine/runner.py's own MacroRunner tests use for
...
from engine.window_resolve import ResolveResult
...
def test_stop_clears_pending_state_via_signal_not_a_direct_call():
    ...
    # thread (webui/bridge.py's stop_macro()), not just the GUI thread
...
def test_poll_reaps_a_run_that_finished_on_its_own():
    ...
    # webui/bridge.py's get_run_state() on every JS polling tick), or the
...
def test_poll_surfaces_a_real_failure_via_last_error():
    ...
    # fired - engine/runner.py's MacroRunner.error is set when the
    ...
    # webui/bridge.py's get_run_state() can surface it to the user.
    controller = make_controller()
    controller.start({'start_node': 'n1', 'nodes': {}}, 'app.exe', '', '.', 'pause_until_focused', False)
    runner = FakeMacroRunner.instances[0]
    runner._running = False
