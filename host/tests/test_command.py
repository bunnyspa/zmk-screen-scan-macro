"""Tests for engine/command.py's HidCommandSink - specifically that the
...
from engine.command import Command, HidCommandSink
...
def test_send_logs_bytes_actually_written(caplog):
    ...
    with caplog.at_level(logging.INFO, logger='engine.command'):
        sink.send(Command(action=wire.ACTION_KEY_PRESS, keycodes=(wire.keycode_for_letter('p'),)))
