...
(host/shared/webui/index.html + graph_editor.js - shared/ because that
whole tree has no Windows-specific dependency and is meant to be reusable
by a non-Windows host later; see host/shared/README.md).
...
Run with: python host/windows/main.py (from the repo root, not host/ or
host/windows/ - see PROFILES_ROOT below, which is cwd-relative).
"""
import logging
import os
import sys
...
# shared/ (host/shared/) has no Windows-specific dependency and holds
# everything a non-Windows host could reuse unchanged (see its own
# README.md) - added to sys.path so this process can still import it by
# flat module name (profile_manager, branch_images, ...), the same way
# every module in this windows/ directory already imports its own flat
# siblings without needing a path insert (this file's own directory is
# put on sys.path automatically by the interpreter for the script it runs).
...
# per-node/per-action logger.info() calls in runner.py (which node is
# being visited, when an action actually fires, focus wait/resume decisions)
# and command.py (bytes actually written per HID report) are silently
# dropped - only ERROR-level failures would ever be seen.
logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s %(name)s: %(message)s')

from hid_link import HidLink, find_device  # noqa: E402
from profile_manager import ProfileError, ProfileManager  # noqa: E402
from pick_controller import PickController  # noqa: E402
from run_controller import RunController  # noqa: E402
from bridge import WebBridge  # noqa: E402

PROFILES_ROOT = os.path.join(os.getcwd(), 'profiles')
INDEX_HTML = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), '..', 'shared', 'webui', 'index.html',
)
