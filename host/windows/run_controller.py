...
pattern runner.py's own MacroRunner already uses for
is_window_focused/focus_window - lets host/tests/windows/test_run_controller.py
...
connection (the same cross-thread marshalling host/windows/hid_link.py's
...
bridge.py's get_run_state() polls from the JS side on a timer - the
one direction (JS calling into Python) already proven safe throughout this
whole migration.
"""
from PyQt5 import QtCore

from overlays import LiveReferenceOverlay, PendingKeyPressOverlay, RegionHighlightOverlay

from command import HidCommandSink
from runner import MacroRunner
from window_capture import WindowCapture
from window_resolve import resolve_target_window as _default_resolve_target_window
...
class RunController(QtCore.QObject):
    ...
    def poll(self):
        """Called from bridge.py's get_run_state() - i.e. on every
        ...
        runner.py's module docstring) - without .stop() ever being
        ...
        if self.macro_runner is not None and not self.macro_runner.is_running():
            ...
            # runner.py's _run() only ever sets .error on the former.
            # Surfaced once via bridge.py's get_run_state(), which
    ...
    def start(self, engine_graph, target_executable, target_window_title,
        ...
        bridge.py). Unlike the old NodeGraphQt desktop app's
    ...
    def stop(self):
        ...
        via bridge.py's stop_macro(), NOT just the GUI thread (&ssm_tog's
    ...
    def confirm(self):
        ...
        bridge.py's confirm_macro() (background API thread) or
    ...
    def _show_branch_overlay_on_gui_thread(self, details):
        ...
        # currently the best match candidate - see runner.py's
