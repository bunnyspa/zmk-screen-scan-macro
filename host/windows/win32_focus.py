"""Win32 window utilities shared by several other modules in this same
windows/ directory (cursor.py, focus.py, overlays.py, pick_controller.py,
run_controller.py) - no single one of those is a more "native" owner of
these calls than another, so this stays its own flat module rather than
living inside any one of them.

find_window_handle() also absorbs what used to be a duplicate
FindWindowW() lookup - cursor.py had its own find_window(), unused
anywhere (confirmed via grep - dead code, removed), and
overlays.py had find_window_handle() with an identical body,
moved here.
...
because cursor.py's get_window_screen_origin()/
get_window_extended_frame_origin() (hwnd -> (x, y) origin only) and
overlays.py's get_window_rect()/get_window_extended_frame_bounds()
