...
click_rect is authored (overlays.py's ClickRegionOverlay, positioned
...
(window_capture.py) produces frames sized to DWM's extended frame
...
match WindowsCapture's convention - matcher.py's per-frame
cropping is correct without any adjustment. Only pick_controller.py's
...
from win32_focus import get_extended_frame_bounds_by_hwnd, get_window_rect_by_hwnd

from command import Command, CommandSink
from monitors import find_containing_monitor, list_monitor_rects
...
def get_window_screen_origin(hwnd) -> tuple[int, int]:
    ...
    the editor's ClickRegionOverlay (overlays.py's get_window_rect())
