...
thread, via command.HidCommandSink. Both share one open device handle -
the app is one process, so there's no need for the
...
def _load_hid():
    if os.name == 'nt' and hasattr(os, 'add_dll_directory'):
        if getattr(sys, 'frozen', False):
            ...
            # hidapi.dll is a flat sibling in this same windows/ directory.
            dll_dir = str(Path(__file__).resolve().parent)
